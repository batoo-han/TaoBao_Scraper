...
Бот автоматически парсит информацию о товарах с Taobao/Tmall/1688/Pinduoduo и создаёт красиво оформленные посты для Telegram с использованием AI.
...
Бот работает с товарами на китайском языке (Taobao/Tmall/1688/Pinduoduo) и генерирует посты на русском языке.
...
**Решения:**
1. Проверьте формат ссылки - поддерживаются Taobao/Tmall/1688/Pinduoduo
2. Убедитесь что TMAPI_TOKEN валиден
3. Для 1688 убедитесь, что указан правильный токен (TMAPI_ALI_TOKEN или TMAPI_TOKEN)
4. Проверьте квоту (100 запросов/день на бесплатном плане)
5. Попробуйте режим отладки: `DEBUG_MODE=True`

### Как настроить парсинг товаров с 1688?

**Настройка:**
1. Получите токен TMAPI на [tmapi.top](https://tmapi.top)
2. В `.env` укажите `TMAPI_TOKEN` (основной токен)
3. Если у вас отдельный токен для 1688, укажите его в `TMAPI_ALI_TOKEN` (иначе оставьте пустым - будет использован основной токен)
4. Отправьте боту ссылку на товар 1688 в формате:
...
**Примечание:** Бот автоматически нормализует URL 1688, извлекая ID товара и формируя правильный формат для API.
...
### Как работает белый и чёрный список пользователей?

В боте реализована система управления доступом:

- **Белый список** — если он включён, бот работает **только** для пользователей, явно добавленных по ID или нику.
- **Чёрный список** — если он включён, пользователи из чёрного списка не могут пользоваться ботом, остальные работают как обычно.
- Если включены **оба списка**, приоритет у чёрного: если пользователь есть и в белом, и в чёрном — доступ запрещён.

Кто может управлять списками:

- основной админ из `ADMIN_CHAT_ID` (переменная в `.env`);
- дополнительные админы из `ADMIN_GROUP_BOT` (список ID через запятую).

Где настраивать:

- через **команду бота** (доступно только админам):  
  в чате с ботом отправьте `/access` — откроется режим управления доступом.  
  В этом режиме доступны текстовые команды:
  - `white on` / `white off` — включить/выключить белый список;
  - `black on` / `black off` — включить/выключить чёрный список;
  - `add white` / `add black` — добавить пользователей (после команды бот попросит список ID/username через запятую);
  - `del white` / `del black` — удалить пользователей из списков;
  - `show` — показать текущее состояние списков и флагов;
- через **Mimi App** (мини‑приложение Telegram) в админском разделе.

Как бот информирует заблокированных:

- при отсутствии доступа пользователь видит понятное сообщение и ник поддержки из `.env` переменной `ACCESS_SUPPORT_USERNAME`, например:  
  «Доступ к боту ограничен. По вопросам доступа напишите @your_support».
