...
### 5.1. Flood control Telegram (retry_after)
**Когда возникает:**
- Telegram возвращает ограничение на отправку сообщений (flood-limit)
- в ошибке присутствует `retry_after` (например, "Retry in 18 seconds")

**Стратегия обработки:**
- если `retry_after <= 60` секунд:
  - пользователю сообщение о блокировке не показываем
  - ждём нужное время и пробуем отправить ещё раз
- если `retry_after > 60` секунд:
  - сохраняем индивидуальную блокировку пользователя в Redis
  - при следующих запросах показываем сообщение:
    ```
    Сервис находится на техническом обслуживании и будет доступен через <время>.
    Приносим Вам извинения за неудобства.
    ```
  - время выводится в человекочитаемом формате (минуты/секунды)

---

### 5.2. Ошибки авторизации Szwego
**Когда возникает:**
- неверный логин/пароль (`invalid_credentials`);
- капча не решена локально (`captcha_failed`);
- SZWEGO временно недоступен (`service_unavailable`);
- неизвестная ошибка (`unknown_error`).

**Коды статуса:**
- `success` — авторизация успешна, cookies и UA сохранены.
- `invalid_credentials` — логин или пароль не подходят.
- `captcha_failed` — не удалось пройти слайдер-капчу (превышен лимит попыток или алгоритм не смог решить).
- `service_unavailable` — SZWEGO не отвечает или недоступен (timeout, network error).
- `unknown_error` — все остальные ошибки (Playwright не установлен, отсутствуют обязательные cookies и т.п.).

**Сообщение пользователю:**
- возвращается в настройках при запуске `🔐 Авторизация в SZWEGO`;
- содержит понятное описание без технических деталей:
  - `invalid_credentials` → «Логин или пароль не подходят. Проверьте данные и попробуйте снова.»
  - `captcha_failed` → «Авторизация закончилась ошибкой: не удалось пройти капчу. Попробуйте позже или обратитесь к администратору.»
  - `service_unavailable` → «SZWEGO временно недоступен. Попробуйте позже.»
  - `unknown_error` → «Авторизация закончилась ошибкой. Попробуйте позже или обратитесь к администратору.»

**Уведомления админу:**
- настраиваются через `SZWEGO_ADMIN_NOTIFY_MODE` (`none` / `all` / `errors`);
- отправляются в чат, указанный в `ADMIN_CHAT_ID` (если задан);
- для ошибок включают тип ошибки, user_id, время и краткое описание (до 500 символов);
- для успеха — краткая информация без чувствительных данных.
