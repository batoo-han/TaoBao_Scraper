...
Верни ТОЛЬКО JSON (без markdown) со структурой:
{
  ...
  "additional_info": {},
  "hashtags": ["2-3шт, одно слово, без # и пробелов. Основа: тип товара + основной материал/ключевое свойство. НЕ детали (принты, узоры, бренды). Используй полные названия (постельноебелье, а не постельное)."],
  "emoji": "один эмодзи"
}
...
- main_characteristics: СТРОГО в следующем порядке:
  - одежда: "Состав", затем "Цвета", затем "Размеры", затем "Уточнения по размерам" (если есть)
  - обувь: "Материал", затем "Цвета", затем "Размеры", затем "Уточнения по размерам" (если есть)
...
- "Уточнения по размерам": список, ТОЛЬКО если в описании есть подробные размеры в сантиметрах (обхват талии, бедра, длина, ширина и т.п.). Пример: ["обхват талии 66-69-72-75 (растягивается)", "бедра 94-97-100-103", "длина брюк 99-100-101-102"]. Если блока "Размеры" нет, но есть подробные размеры - добавь их в "Размеры" вместо "Уточнения по размерам".
